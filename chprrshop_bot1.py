...
TEXTS = {
    ...
    "uk": {
        ...
        "stars_title": "⭐️ <b>Купівля Telegram Stars</b>\n\nОберіть кількість зірок або введіть свою:",
        ...
        "ask_receipt": "📎 <b>Надішліть чек або скріншот оплати</b>\n\nПрикріпіть photo або документ з підтвердженням оплати:",
...
async def send_main_page(chat_id: int, user_id: int, state: FSMContext):
    ...
    # Сбрасываем шаги FSM, но сохраняем язык в базе данных сессии
    ...
    if await is_subscribed(bot, user_id):
        ...
        if cached_photo_id:
            try:
                sent = await bot.send_photo(chat_id, photo=cached_photo_id, caption=main_text, reply_markup=main_keyboard(lang), parse_mode="HTML")
            except Exception:
                cached_photo_id = None
...
async def back_from_username_stars(cb: types.CallbackQuery, state: FSMContext):
    ...
    if is_custom:
        ...
        sent = await replace_message(
            ...
            TEXTS[lang]["stars_custom_prompt"],
            reply_markup=back_kb("menu_stars", lang)
        )
        await state.update_data(last_msg_id=sent.message_id)
    else:
...
async def stars_pick(cb: types.CallbackQuery, state: FSMContext):
    ...
    sent = await replace_message(
        bot, cb.message.chat.id, old,
        TEXTS[lang]["stars_selected"].format(amount=amount, price=price),
        reply_markup=back_kb("back_from_username_stars", lang)
    )
    await state.update_data(last_msg_id=sent.message_id)
    await cb.answer()
...
async def stars_custom_amount(msg: types.Message, state: FSMContext):
    ...
    sent = await replace_message(
        bot, msg.chat.id, old,
        TEXTS[lang]["stars_custom_qty"].format(amount=amount, price=price),
        reply_markup=back_kb("back_from_username_stars", lang)
    )
    await state.update_data(last_msg_id=sent.message_id)
...
async def stars_username(msg: types.Message, state: FSMContext):
    ...
    if not re.match(r'^[a-zA-Z0-9_]+$', input_text):
        sent = await replace_message(
            bot, msg.chat.id, old,
            TEXTS[lang]["username_error"].format(amount=amount, price=price),
            reply_markup=back_kb("back_from_username_stars", lang)
        )
        await state.update_data(last_msg_id=sent.message_id)
        return
    ...
    sent = await replace_message(
        bot, msg.chat.id, old,
        TEXTS[lang]["verify_title"].format(item=item_name, price=price, recipient=recipient_display),
...
async def user_confirm_stars(cb: types.CallbackQuery, state: FSMContext):
    ...
    sent = await replace_message(
        bot, cb.message.chat.id, old,
        TEXTS[lang]["order_sent"].format(item=item_name, price=price, recipient=recipient_display),
        reply_markup=back_kb("back_main", lang)
    )
    await state.update_data(last_msg_id=sent.message_id)
    await cb.answer()
...
async def premium_pick(cb: types.CallbackQuery, state: FSMContext):
    ...
    sent = await replace_message(
        bot, cb.message.chat.id, old,
        TEXTS[lang]["premium_selected"].format(label=label, price=price),
        reply_markup=back_kb("menu_premium", lang)
    )
    await state.update_data(last_msg_id=sent.message_id)
    await cb.answer()
...
async def premium_username(msg: types.Message, state: FSMContext):
    ...
    if not re.match(r'^[a-zA-Z0-9_]+$', input_text):
        sent = await replace_message(
            bot, msg.chat.id, old,
            TEXTS[lang]["premium_username_error"].format(label=label, price=price),
            reply_markup=back_kb("menu_premium", lang)
        )
        await state.update_data(last_msg_id=sent.message_id)
        return
...
async def user_confirm_premium(cb: types.CallbackQuery, state: FSMContext):
    ...
    await log_action(client_id, client_name, "order_premium_created", {
        "duration": label_ru, "price": price, "target_user": recipient_display
    })
...
# ─── ADMIN CORE HANDLERS (READS CLIENT LANG ENVIRONMENT) ──────────────────────
...
async def admin_confirm_cb(cb: types.CallbackQuery):
    ...
    # Извлекаем язык из сессии клиента через общий менеджер контекстов FSM
    ...
    await bot.send_message(
        user_id,
        TEXTS[client_lang]["admin_confirmed"].format(price=price, CARD_NUMBER=CARD_NUMBER),
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=TEXTS[client_lang]["btn_i_paid"], callback_data="send_receipt")]
        ]),
        parse_mode="HTML"
    )
...
async def admin_input_reason(msg: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        user_id,
        TEXTS[client_lang]["admin_declined"].format(reason=reason, SUPPORT_USERNAME=SUPPORT_USERNAME),
        parse_mode="HTML"
    )
    try:
        await bot.edit_message_text(chat_id=ADMIN_ID, message_id=old_msg_id, text=old_text + f"\n\n❌ <b>Заказ отклонён. Причина:</b> {reason}", parse_mode="HTML")
    except Exception: pass
...
async def admin_complete_order(cb: types.CallbackQuery):
    ...
    await bot.send_message(
        user_id,
        TEXTS[client_lang]["admin_completed"],
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=TEXTS[client_lang]["btn_leave_review"], callback_data="user_leave_review")]
        ]),
        parse_mode="HTML"
    )
